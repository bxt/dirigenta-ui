...
struct LightColorControls: View {
    ...
    init(
        ...
        _colorTempValue = State(
            initialValue: Double(attrs.colorTemperature ?? 2700)
        )
        ...
        HStack {
            ...
            .onReceive(
                NotificationCenter.default.publisher(
                    for: NSColorPanel.colorDidChangeNotification
                )
            ) { _ in
    ...
    private func showColorPanel() {
        ...
        if let window = NSApp.keyWindow {
            panel.setFrameTopLeftPoint(
                NSPoint(x: window.frame.maxX + 8, y: window.frame.maxY)
            )
        }
    ...
    private func applyColor() {
        guard let ns = NSColor(selectedColor).usingColorSpace(.deviceRGB) else {
            return
        }
        var h: CGFloat = 0
        var s: CGFloat = 0
